...
func (f *FacebookProvider) ReactToPost(cookie string, postURL string, reactionType string, docId string) (string, error) {
	if docId == "" {
		return "", errors.New("CHƯA CẤU HÌNH DOC_ID! Vui lòng vào Cài đặt (Settings) và điền dãy số GraphQL React Doc_ID lấy từ trình duyệt của bạn.")
	}
	...
	if strings.Contains(fbBody, `"severity":"CRITICAL"`) || strings.Contains(fbBody, `"error":`) {
