<template>
  <div class="space-y-6">
      ...
      <table v-else class="w-full text-left text-sm">
        <thead class="bg-gray-50 text-gray-600 font-medium border-b border-gray-200">
          <tr>
            ...
            <th class="py-3 px-4">Tên gợi nhớ</th>
            <th class="py-3 px-4">UID (c_user)</th>
            <th class="py-3 px-4">Giấy phép (Doc_ID)</th>
            <th class="py-3 px-4">Ngày thêm</th>
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="(acc, index) in accounts" :key="acc.uid" class="hover:bg-gray-50 transition-colors">
            ...
            <td class="py-3 px-4 text-gray-500 text-xs font-mono">{{ acc.graphqlLikeDocId || 'Mặc định' }}</td>
            <td class="py-3 px-4 text-gray-500">{{ acc.createdAt }}</td>
            <td class="py-3 px-4">
    ...
    <!-- Add Account Modal -->
    <div v-if="showAddModal" class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-fade-in-up">
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea v-model="newAccCookie" rows="3" placeholder="c_user=1000...; xs=...;" 
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Facebook GraphQL Doc_ID (Tùy chọn)</label>
            <input v-model="newAccDocId" type="text" placeholder="Ví dụ: 7448648608581699" 
            ...
            <p class="text-xs text-gray-500 mt-1">Nếu bỏ trống, hệ thống sẽ sử dụng Doc_ID mặc định ở phần Cài Đặt.</p>
          </div>
        </div>
...
async function submitAdd() {
  ...
  addError.value = ""
  try {
    await AddAccount(newAccName.value, newAccCookie.value, newAccDocId.value)
    await fetchAccounts()
